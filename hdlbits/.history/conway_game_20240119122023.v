            ...
            //row 0: left i+2, center i+1, right i;
                ...
                always @(*) begin
                    neighbor = {regfile[i_mod16_upper*16+j_mod16_left],    regfile[i_mod16_upper*16+j],     regfile[i_mod16_upper*16+j_mod16_right],
                                regfile[i*16+j_mod16_left],                                                 regfile[i*16+j_mod16_right],
                                regfile[i_mod16_lower*16+j_mod16_left],     regfile[i_mod16_lower*16+j],    regfile[i_mod16_lower*16+j_mod16_right]};  
                end

                always @(*) begin
                    num_neighbor = neighbor[7]+neighbor[6]+neighbor[5]+neighbor[4]+neighbor[3]+neighbor[2]+neighbor[1]+neighbor[0];
                end
            end
        end
    endgenerate
